...
class Mundial_Salsa_Mega_Menu_Walker extends Walker_Nav_Menu {
    ...
    public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
        ...
        if ( in_array( 'mega-menu-pro', $classes ) ) {
            ...
            // Standard Menu Item - Ensure it has the same h-full and styling
            $output .= $indent . '<li class="menu-item h-full flex items-center ' . esc_attr( implode( ' ', $classes ) ) . '">';
            $output .= '<a href="' . esc_url( $item->url ) . '" class="nav-link flex items-center h-full font-bold uppercase tracking-wider text-sm hover:text-[var(--mds-primary)] transition-colors">' . esc_html( $item->title ) . '</a>';
            $output .= '</li>';
        }
    }
}
